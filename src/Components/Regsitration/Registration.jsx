...
const Registration = () => {
  ...
  const handelReg = (e) => {
    ...
    if (!passwordRegex.test(password)) {
      setPasswordError("Password must include at least one uppercase letter, one lowercase letter, and be at least 6 characters long.");
      return;
    } else {
      setPasswordError(""); 
    }
      ...
      .then((result) => {
        ...
        updateData({ displayName: name, photoURL: imgUrl }).then(() => {
          if (result.user) {
            ...
            toast.success(`Thank you, ${name}! Your registration was successful.`);
     ...
     <div className="max-w-md mx-auto p-6 bg-transparent rounded shadow mt-4 ">
      ...
      <form onSubmit={handelReg}>
        ...
        <div className="mb-4">
          ...
          <input type="text" id="name" ref={nameInputRef}  name="name" required className="w-full px-3 py-2 border rounded" />
        </div>
        <div className="mb-4">
        ...
        <div className="mb-4">
          ...
          <input type="url" id="photoUrl" name="imgUrl" className="w-full px-3 py-2 border rounded" />
        </div>
        <div className="mb-4">
