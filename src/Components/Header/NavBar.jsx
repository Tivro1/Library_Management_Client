...
{/* Logo in Phone view */}
                ...
                <div className="w-[30%] h-full flex gap-3 justify-center items-center">
                    {!user && (
                        <div>
                            <NavLink to="/reg">
                                <button className="regDiv  p-2 hover:bg-red-100 bg-green-100 rounded-md btn-custom font-bold text-black">Registration</button>
                            </NavLink>
                            <NavLink to="/login">
                                <button className="regDiv  p-2 hover:bg-gray-100 bg-red-100 rounded-md btn-custom font-bold text-black">Login</button>
                            </NavLink>
                        </div>
                    )}
                    {user && (
            ...
            {dropdownVisible && (
                <div className="md:hidden absolute top-16 left-1/2 -translate-x-1/2 bg-white w-11/12 py-3 rounded-lg shadow-lg z-10">
                    <NavLink to="/" className="block text-center py-2 text-lg font-bold text-gray-800">Home</NavLink>
                    {user && (
                        <>
                            <NavLink to="/allbooks" className="block text-center py-2 text-lg font-bold text-gray-800">All Books</NavLink>
                            <NavLink to="/addbooks" className="block text-center py-2 text-lg font-bold text-gray-800">Add Books</NavLink>
                            <NavLink to="/borrowed" className="block text-center py-2 text-lg font-bold text-gray-800">Borrowed Books</NavLink>
                        </>
                    )}
                    {!user && (
                        <>
                            <NavLink to="/reg" className="block text-center py-2 text-lg font-bold text-gray-800">Registration</NavLink>
                            <NavLink to="/login" className="block text-center py-2 text-lg font-bold text-gray-800">Login</NavLink>
                        </>
                    )}
                </div>
            )}
