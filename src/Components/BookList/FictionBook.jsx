...
const FictionBook = () => {
    const fictionBookData = useLoaderData();
    console.log(fictionBookData);
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {
                    fictionBookData.map(res => 
                        <div key={res._id} className="bg-white p-4 rounded-lg shadow-md hover:shadow-lg transition-shadow">
                            ...
                            <button onClick={()=> handelDetails(res.category,res._id)} className="mt-4 bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 transition-colors">
                                Details
                            </button>
                        </div>
                    )
