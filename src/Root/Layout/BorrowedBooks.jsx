  ...
  const handleReturnBook = async (bookId, category) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `https://libraymanagement-nu.vercel.app/${normalizedCategory}/${bookId}/return`
      );
      if (response.status === 200) {
      
        
        // Delete the book after successful return
        await axios.delete(`https://libraymanagement-nu.vercel.app/borrow/${bookId}`, {
          headers: {
      ...
      console.error("Error returning the book:", error.response || error.message);
